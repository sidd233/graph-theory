...
\begin{definition}[2-Kernel]
  Let $D=(V,A)$ be a digraph. A subset $N\subseteq V(D)$ is a \textbf{2-kernel} of $D$ if $N$ is both independent and 2-dominant. Equivalently, $N$ is a 2-kernel if
  ...
  This is stronger than the classical (von Neumann--Morgenstern) kernel, which only requires each outside vertex to reach $N$ by \emph{some} arc; a 2-kernel requires \emph{at least two}.
\end{definition}

\bigskip

\begin{example}
  In the digraph below, $N=\{v_1,v_3\}$ is a 2-kernel: $v_1$ and $v_3$ are not joined by an arc, and each of $v_2,v_4$ has an arc into both $v_1$ and $v_3$, so each outside vertex has two arcs into $N$.
...
\begin{remark}
  If $N$ is a 2-kernel of $D$, then $N$ is also an ordinary kernel of $D$ (independence is unchanged, and having two arcs into $N$ certainly gives at least one). The converse fails: $N=\{v_1,v_3\}$ ceases to be a 2-kernel if, say, the arc $v_4\to v_3$ is deleted, even though it remains an ordinary kernel as long as $v_4\to v_1$ survives.
\end{remark}
