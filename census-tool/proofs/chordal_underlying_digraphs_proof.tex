...
holds in every 2-d kernel of $D$ — not merely for whatever candidate the process is
building.
\end{lemma}
...
2-d kernel $S$; since $S$ is independent and there is an arc between $u$ and $v$ (in
...
excluded from every 2-d kernel $S$. Take any 2-d kernel $S$ and suppose $v \notin S$.
...
contradicts the firing condition. So $v \in S$ for every 2-d kernel $S$.
\end{proof}
...
has at most one 2-d kernel, and 2-D KERNEL is solvable in polynomial time on such
...
in \emph{every} 2-d kernel of $D$ — so if $D$ has a 2-d kernel at all, it is exactly the
