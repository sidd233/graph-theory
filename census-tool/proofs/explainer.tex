...
\title{2-Kernels in Graphs and Digraphs: A Complete Walkthrough of FINDINGS.md}
...
\subsection{Independent sets, domination, and the 2-kernel}
...
\begin{definition}[2-kernel — the object of this whole project]
A set $S \subseteq V$ is a \textbf{2-kernel} of a digraph $D$ if:
...
A 2-kernel is automatically a \textbf{maximal} independent set of the underlying
...
has \textbf{no} 2-kernel when $n$ is odd, and \textbf{exactly two} when $n$ is even ---
...
Because a 2-kernel is always a maximal independent set of the underlying graph
...
on a partial labelling of the vertices as \texttt{IN} (definitely in every 2-kernel),
...
\texttt{IN}. \emph{Why:} if $v$ were outside a 2-kernel $S$, then $S$ would need two
...
\texttt{CONFLICT} — no 2-kernel can exist. \emph{Why:} $v$ can never find two
...
one of three verdicts: \texttt{CONFLICT} (proved no 2-kernel exists), \texttt{SOLVED}
(every vertex decided, and the \texttt{IN} set is verified as an actual 2-kernel), or
...
though (checked by the full solver) this graph in fact has no 2-kernel at all.
\end{example}
...
true answer is ``no 2-kernel''. Not one case was found where the closure's
...
\texttt{count\_2kernels} & how many distinct 2-kernels it has \\
\texttt{min\_size}, \texttt{max\_size} & smallest and largest 2-kernel, where any exist \\
...
than one family without its expensive properties (the 2-kernel count, in particular)
...
\section{Result 1: how common are 2-kernels? (FINDINGS.md \S2, ``E1'')}
\label{sec:e1}

Across \textbf{all 288{,}267 graphs on at most 9 vertices} (an exhaustive count, not a
sample), \textbf{125{,}439 (43.5\%)} have a 2-kernel. The rate barely moves with $n$
(42.9\% at $n\le 7$ versus 43.5\% at $n \le 9$) but swings enormously by class:

\begin{center}
\begin{tabular}{@{}lrrl@{}}
\toprule
class & members & with a 2-kernel & smallest members without one \\
\midrule
...
every class that allows an edge at all, and a 2-kernel needs at least 2 members, which
$K_2$'s independence number of 1 can't supply. The only class in this table with
\emph{no} obstruction at all is DAGs — every one tested has a 2-kernel, which
...
\emph{at most one} 2-kernel, and deciding 2-kernel existence on chordal graphs is
...
candidate 2-kernel the closure could possibly produce, so there is at most one.
\end{proof}
...
again there is at most one 2-kernel.
\end{theorem}
...
maximum number of 2-kernels found on either class is exactly 1, while cographs and
bipartite graphs reach as many as 4.

\subsection{Where this sits in the literature}
\label{sec:literature}

A 2-kernel fits a general framework of Telle (1994): a $(\sigma,\rho)$-\emph{dominating
...
in $S$ from the set $\rho$. A 2-kernel is exactly $\sigma = \{0\}$ (independence),
$\rho = \{2,3,4,\dots\}$ (2-domination).

\begin{theorem}[Telle, 1994]
\label{thm:telle}
Deciding whether a general graph has a $(\{0\}, \{q, q{+}1, q{+}2, \dots\})$-dominating
set is NP-complete, for every $q \ge 2$. Taking $q = 2$ shows 2-KERNEL is NP-complete
on general graphs.
\end{theorem}
...
2-kernel's specific $\rho = \{2,3,\dots\}$ (an infinite, ``cofinite'' set, rather than
...
has no 2-kernel, and Example~\ref{ex:stuck} above traced exactly why forcing cannot
...
every vertex of $Q$ has $d^+ \ge 2$, then $P$ is a 2-kernel.
\end{theorem}
...
$\delta^+(D) \ge 2$, then $D$ has two disjoint 2-kernels.
\end{theorem}
...
least $\delta^+(D) \ge 2$ of them. So $V_{1-i}$ is a 2-kernel, for each $i \in
\{0,1\}$ — giving two 2-kernels, $V_0$ and $V_1$, which are disjoint since they
partition $V$.
\end{proof}
...
only two candidate 2-kernels are $\{1,3\}$ and $\{2,4\}$.
...
possibilities fail, so no 2-kernel exists.
\end{example}
...
connected, exactly 84 (66.7\%) still have a 2-kernel — so Example~\ref{ex:C} is a
...
and all-cycles-even, \textbf{all 81} have a 2-kernel, exactly as Theorem~\ref{thm:B}
guarantees.
...
\begin{theorem}
No oriented graph on fewer than 8 vertices with $\delta^+ \ge 2$ has a 2-kernel, and
$n = 8$ is attained.
\end{theorem}

\begin{proof}
Let $D$ be an oriented graph with $\delta^+ \ge 2$, and suppose it has a 2-kernel $J$
...
\]
every vertex has out-degree exactly 2, no digon is present, and the 2-kernels are
...
The question tested: does forbidding short cycles make a 2-kernel more likely to
exist, among cubic graphs? Over seeded random samples at $n = 10, 12, \dots, 20$:

\begin{center}
\begin{tabular}{@{}lrrr@{}}
\toprule
family & isomorphism classes & with a 2-kernel & fraction \\
\midrule
...
(degree-3) graphs, for the idea that avoiding short cycles helps guarantee a 2-kernel.
...
\begin{theorem}
For every graph $G$, the original vertex set $V(G)$ is a 2-kernel of its subdivision
$S(G)$.
\end{theorem}
...
$S(G) = G$ and $V(G)$ is the whole vertex set, a 2-kernel vacuously.)
\end{proof}

This gives a free, infinite family of graphs guaranteed to have a 2-kernel — checked
...
\begin{theorem}
A DAG has at most one 2-kernel. Moreover, there is a linear-time procedure that either
outputs that unique 2-kernel, or correctly certifies that none exists.
\end{theorem}
...
and $k=1$. \textbf{Neither status is available.} Since every 2-kernel must give
$v_i$ one of the two statuses, and by the inductive hypothesis every 2-kernel must
already agree with the statuses fixed on $N^+(v_i)$ so far, this means \emph{no
2-kernel of $D$ exists at all}. Halt and report non-existence.
...
permanently decided. So $S$ is a genuine 2-kernel, and since every step above was
...
DAGs with a 2-kernel reported below. There is no separate ``independence check''
...
\begin{corollary}
Deciding whether a DAG has a 2-kernel is not just polynomial, it is linear time —
answering, negatively, whether 2-KERNEL might be NP-complete on DAGs the way finding
...
pattern in the data: the fraction of DAGs with a 2-kernel falls steadily as $n$ grows
...
and uniqueness fails too (up to 4 distinct 2-kernels appear).
...
$D$ completely, so $D$ has at most one 2-kernel, and 2-KERNEL is solvable in
...
it turns $\delta^+ \ge 2$ from something that helps build a 2-kernel (as in Theorems A
...
\item Counting the exact number of 2-kernels (\texttt{count\_2kernels}) uses the
...
$S$ & a candidate or actual 2-kernel \\
