...
\title{2-Kernels in Graphs and Digraphs: Every Result, Proved or Disproved}
...
Throughout, a \textbf{2-kernel} of a digraph $D$ is a set $S \subseteq V$ that is
...
out-degree in $D$. Deciding 2-kernel existence on general graphs is NP-complete
...
\item \textbf{Oriented graphs, tight bound} — no 2-kernel below 8 vertices once
...
\item \textbf{DAGs} — unique 2-kernel, decidable in linear time. \textit{Proved.}
...
vertex of $Q$ has $d^+ \ge 2$, then $P$ is a 2-kernel.
\end{theorem}
...
$V_0$ and $V_1$ are 2-kernels, and they're disjoint since they partition $V$.
\end{proof}
...
digraph has \textbf{no} 2-kernel.
\end{example}

\begin{proof}[Why no 2-kernel exists]
...
(66.7\%) still lack a 2-kernel or have one only by luck — this is a typical failure
...
status this fixes holds in \emph{every} 2-kernel of $G$, not just some candidate.
\end{proposition}
...
vertex \texttt{UNKNOWN}. Consequently a chordal graph has at most one 2-kernel, and
2-KERNEL is solvable in polynomial time on chordal graphs.
\end{theorem}
...
the resulting set is the \emph{only} possible 2-kernel of $G$.
\end{proof}

\emph{Confirmed exhaustively} on all 17{,}175 chordal graphs up to 9 vertices: 0
\texttt{UNDECIDED}, maximum 2-kernel count 1.
...
\begin{theorem}
A split graph $G=(C,I)$ has a 2-kernel if and only if either
...
unless $S=I$ exactly. (Every 2-kernel is a maximal independent set, since anything
...
by brute force) this graph in fact has no 2-kernel. This does not contradict Wloch's
...
fact has no 2-kernel at all (Section 12's cycle fact).
\end{proof}
...
(9.5\%), maximum 2-kernel count rises to 4. The conjecture fails at the very next
step past forests.
...
2-kernel, with no restriction on out-degree at all.
\end{theorem}
...
\begin{theorem}
Let $D$ be oriented with $\delta^+(D)\ge2$. If $D$ has a 2-kernel, $D$ has at least
8 vertices, and 8 is attained.
\end{theorem}

\begin{proof}
Let $J$ be a 2-kernel, $j=|J|$, $B=V\setminus J$, $t=|B|>0$. Writing $d_x$ for the
...
digon, and both $J$ and $B$ are 2-kernels.
\end{example}
...
\emph{exactly} 2 (not merely $\ge2$) and which have a 2-kernel of size 4, there are
...
a valid 2-kernel (its independence and domination depend only on arcs touching $J$).
...
\begin{theorem}
A DAG has at most one 2-kernel, decidable in linear time.
\end{theorem}
...
the $k\ge2$ condition that placed it there. So $S$ is a genuine 2-kernel, and since
every step is forced with no branching, the only one.
\end{proof}

\emph{Confirmed} on all 20{,}237 DAGs up to 7 vertices: max count 1, 0
\texttt{UNDECIDED}. The fraction of DAGs \emph{with} a 2-kernel falls steadily
...
\begin{theorem}
For every graph $G$, $V(G)$ is a 2-kernel of $S(G)$.
\end{theorem}
...
Does forbidding short cycles make a 2-kernel more likely, among 3-regular graphs?
