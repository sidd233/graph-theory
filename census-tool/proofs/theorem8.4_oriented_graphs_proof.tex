...
$u\to v$, $v\to u$ is present) with $\delta^+(D) \ge 2$. If $D$ has a 2-kernel, then
$D$ has at least 8 vertices, and 8 is attained.
\end{theorem}

\begin{proof}
Let $J$ be a 2-kernel of $D$, $j = |J|$, and $B = V \setminus J$, $t = |B| = n - j > 0$.
...
and $B = \{4,5,6,7\}$ are both 2-kernels.
\end{example}
...
\emph{exactly} 2, and which have a 2-kernel of size 4, there are exactly 2
...
2-kernel (all 90 pass); and grouping the 90 by graph isomorphism yields exactly 2
...
a valid 2-kernel — $J$'s independence and 2-domination depend only on arcs
...
of non-isomorphic 8-vertex oriented graphs with $\delta^+\ge2$ and a size-4 2-kernel,
